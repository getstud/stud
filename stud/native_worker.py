...
def measurement(model,manifest,args):
    ...
    for target in args['targets']:
        ...
        object_id=target.get('object_id');point=target.get('point_mm')
        ...
        if not isinstance(point,list) or len(point)!=3 or not all(isinstance(n,(int,float)) and math.isfinite(n) for n in point):
            raise StudError('invalid_measurement','A picked point requires finite world coordinates in millimeters.')
        ...
        maximum=float(target.get('max_snap_mm',manifest['settings']['linear_tolerance_mm']*2+tolerance))
        ...
        targets.append(resolved);picks.append(dict(object_id=object_id,input_point_mm=point,native_point_mm=resolved,snap_distance_mm=query.Value()))
    kind=args['kind']
    if kind in ('point_distance','length'):
        ...
        value=math.dist(a,b);units='mm';evidence=dict(start=a,end=b,operation='native reference distance')
    else:
        requirement=dict(kind=kind,targets=targets,threshold=0,tolerance=tolerance)
        value,_,evidence=measure_requirement(model,requirement);units=evidence.get('units','mm')
    return dict(project_id=manifest['project_id'],source_id=manifest['source_id'],build_id=manifest['build_id'],
        status='measured',kind=kind,value=value,units=units,tolerance_mm=tolerance,evidence=evidence,picks=picks)
