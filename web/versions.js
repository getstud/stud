...
export function installVersions({showWorkspace,refreshModel}){
 ...
 function selectPart(id){const row=comparison.objects.find(row=>row.id===id);$('comparison-selected').textContent=`${row.right?.label||row.left?.label||id}\n${id}\n${row.changes.join(', ')}${row.left?.blank?.size_mm?`\nA blank: ${row.left.blank.size_mm.map(v=>v.toFixed(2)).join(' × ')} mm`:''}${row.right?.blank?.size_mm?`\nB blank: ${row.right.blank.size_mm.map(v=>v.toFixed(2)).join(' × ')} mm`:''}`;}
