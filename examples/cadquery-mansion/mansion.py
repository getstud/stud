"""Mixed two-storey workload: 5,488 sq ft of modular floors around a courtyard.
...
def residence(model, *, bay=168, window_width=40, local_window_width=48,
              wall_height=108, slope=.45, bore_diameter=.5, rotation=0,
              include_guest=True):
        ...
        for level in range(2):
            ...
            for index,room in enumerate(names):
                ...
                floor=floor_frame(model,object_id=module+'.floor',parent=module,length=bay,depth=bay,
                    stair_opening=dict(x=60,y=28,width=40,depth=88) if wing=='north' and room=='stairs' and level==1 else None)
                floors.append(floor['id'])
                walls={};d=3.5;h=wall_height
                for side,origin_,angle_ in [('front',(0,0,6.25),0),('back',(bay,bay,6.25),180)]:
                    width=local_window_width if wing=='west' and room=='kitchen' and side=='front' and level==0 else window_width
                    opening=dict(id='window',x=(bay-width)/2,width=width,sill=36,height=44)
                    bore={'stud.at_16':dict(bore=dict(x=.75,z=28,diameter=bore_diameter))} if wing=='west' and room=='kitchen' and side=='front' and level==0 else None
                ...
                for side,origin_,angle_,outside in [('left',(0,bay-d,6.25),-90,index==0),('right',(bay,d,6.25),90,index==len(names)-1)]:
                    openings=[] if outside else [dict(id='passage',x=56,width=44,height=84)]
    ...
    model.notes += ['Architectural framing and performance study: 14 room bays on each of two storeys, 5,488 square feet of floor platforms at the default dimensions.',
    ...
    return dict(floors=floors,roofs=roofs,feature_walls=feature_walls,
                area_sqft=len(floors)*bay*bay/144)
