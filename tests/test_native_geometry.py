...
from stud.construction import roof_joint, workbench, rotated_opening
...
class NativeGeometryTests(unittest.TestCase):
    def test_rotated_workbench_drawings_follow_its_placement_and_section_plane(self):
        ...
        model=Model('Rotated bench')
        workbench(model,dog_hole=None,location=cq.Location(cq.Vector(254,508,0),cq.Vector(0,0,1),90))
        self.assertTrue(check_model(model)['all_passed'])
        section=model.drawings['bench.section']['section']
        for actual,expected in zip(section['origin'],[254,508+914.4,0]):self.assertAlmostEqual(actual,expected)
    ...
    def test_support_projects_contact_in_the_declared_bearing_direction(self):
        model=Model('Bearing direction');box=cq.Workplane('XY').box(10,10,10,centered=(False,False,False))
    ...
    def test_end_members_do_not_overlap_and_sheet_seams_follow_custom_spacing(self):
        for builder in (framed_wall,floor_frame):
            model=Model('End boundary');part=builder(model,object_id='component',length=2470,spacing=600)
            ...
            self.assertEqual(positions,[0,1200,2400])
        model=Model('Actual missing backing');part=framed_wall(model,object_id='wall',length=3048,spacing=600)
        model.shapes['wall.stud.at_2400']['world']=model.shapes['wall.stud.at_2400']['world'].moved(cq.Location(cq.Vector(50,0,0)))
        findings=check_model(model)['findings']
        self.assertTrue(any(f['kind']=='panel_edge_support' and f['status']=='failed' and f['measured']>2000 for f in findings))

    def test_purchase_audit_detects_missing_material_stale_cut_and_hardware(self):
        from copy import deepcopy
        model=Model('Purchases');workbench(model);manifest=model.export()
        self.assertEqual(audit_fabrication(manifest),[])
        wrong=deepcopy(manifest);wrong['demands'][0]['cuts_mm'][0]['length_mm']=10
        ...
        wrong=deepcopy(manifest);wrong['demands'][0]['specification']['section_mm']=[1,1]
        self.assertIn('incompatible_stock_section',{f['category'] for f in audit_fabrication(wrong)})
        corner=Model('Counted plate');rotated_opening(corner);wrong=corner.export()
        next(d for d in wrong['demands'] if d['product_id']=='steel.plate.70x70x6')['quantity']=0
    ...
    def test_panel_bearing_requirement_must_resolve_and_custom_opening_stays_supported(self):
        ...
        model=Model('Custom spacing');framed_wall(model,object_id='wall',length=3048,spacing=600,
            openings=[dict(id='window',x=1200,width=850,sill=900,height=900)])
    ...
    def test_corner_profile_bore_and_birdsmouth_use_reproducible_local_coordinates(self):
        model=Model('Mirror');rotated_opening(model,mirrored_detail=True)
        self.assertEqual(audit_fabrication(model.export()),[])
        operations=model.objects['opening.corner_detail']['blank']['operations']
        self.assertEqual(operations[1]['center_mm'],[55,15])
        self.assertIn((0,25),operations[0]['profile_mm'])
        from stud.plans import project_vector,operation_text
        drawing,_=project_vector(model,model.drawings['opening.corner_plan'],1)
        self.assertAlmostEqual(drawing.width,70*72/25.4,places=5)
        roof=Model('Stock datum');roof_joint(roof)
        cut=roof.objects['roof_joint.rafter']['blank']['operations'][1]
        self.assertAlmostEqual(cut['stock_seat_endpoints_mm'][0][2],44.5/math.sqrt(1.25))
        self.assertAlmostEqual(cut['stock_seat_endpoints_mm'][1][2],0)
    ...
    def test_stock_frame_for_sloped_rafter_is_rectangular_and_measured(self):
        model=Model('Sloped');roof_joint(model,slope=.5,run=500,seat=89)
        result=check_model(model)
        self.assertTrue(result['all_passed'],result)
        blank=model.objects['roof_joint.rafter']['blank']['size_mm']
        self.assertEqual(blank[0],38);self.assertEqual(blank[2],140)
        self.assertAlmostEqual(blank[1],(500+140*math.sin(math.atan(.5)))/math.cos(math.atan(.5)))
        expected=38*(500*140/math.cos(math.atan(.5))-.5*.5*89**2)
        ...
        self.assertAlmostEqual(bearing['measured'],38*89,places=5)
    ...
    def test_bored_solid_distance_uses_hole_not_bounds(self):
        ...
        self.assertAlmostEqual(model.objects['plate']['volume_mm3'],200000-math.pi*100*20,places=5)
    ...
    def test_batch_rollback_reference_replacement_and_frozen_blank(self):
        model=Model('Rollback');blank={'size_mm':[10,20,30],'operations':[{'kind':'square_cut'}]}
        shape=cq.Workplane('XY').box(10,20,30,centered=(False,False,False))
        model.part('a',shape,blank=blank)
        blank['size_mm'][0]=1
        self.assertEqual(model.objects['a']['blank']['size_mm'][0],10)
    ...
    def test_wall_opening_removes_framing_and_panels_with_independent_quantities(self):
        model=Model('Door wall')
        wall=framed_wall(model,object_id='wall',length=3048,openings=[dict(id='door',x=1066.8,width=914.4,height=2032)])
        self.assertEqual(len(wall['parts']),17)
        self.assertEqual(len(wall['panels']),3)
        opening=cq.Workplane('XY').box(914.4,110,2032,centered=(False,False,False)).translate((1066.8,-15,0)).val()
        ...
        panel_volume=sum(model.objects[part]['volume_mm3'] for part in wall['panels'])
        self.assertAlmostEqual(panel_volume,((3048-6)*2438.4-(914.4-3)*2032)*12.7,places=4)
        result=check_model(model)
        self.assertTrue(result['all_passed'],result)
    ...
    def test_shared_wall_regeneration_preserves_unrelated_bore_exception(self):
        def make(width):
            model=Model('Window')
            wall=framed_wall(model,object_id='wall',length=3048,openings=[dict(id='window',x=1200,width=width,sill=900,height=900)],
                exceptions={'stud.at_406_4':{'bore':{'x':19,'z':500,'diameter':12}}})
            return model,wall
        first,a=make(650);second,b=make(850)
        key='wall.stud.at_406_4'
        ...
        expected=38*89*(2438.4-114)-math.pi*6**2*89
        self.assertAlmostEqual(second.objects[key]['volume_mm3'],expected,places=4)
        self.assertTrue(check_model(second)['all_passed'])

    def test_platform_actual_panels_and_stair_cut(self):
        model=Model('Floor');floor=floor_frame(model,object_id='floor',length=3048,depth=2438.4)
        self.assertEqual(len(floor['parts']),11);self.assertEqual(len(floor['panels']),3)
        self.assertAlmostEqual(sum(model.objects[p]['volume_mm3'] for p in floor['panels']),(3048-6)*2438.4*19.05,places=4)
        self.assertTrue(check_model(model)['all_passed'])
        cut_model=Model('Stair')
        floor=floor_frame(cut_model,object_id='floor',length=3048,depth=2438.4,stair_opening=dict(x=850,y=650,width=900,depth=1400))
        void=cq.Workplane('XY').box(900,1400,160,centered=(False,False,False)).translate((850,650,0)).val()
